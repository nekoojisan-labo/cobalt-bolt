...
- ユーザー指摘「造形がぶっ壊れてない？」→ 全コマ精査で確認: 生成キャラは肘ジュエル浮遊・輪郭ギザつき・腕融合の板状アーティファクトが実用基準未達。修復を10回超イテレーションした結論:
  ...
  - 試行と結果の台帳: 剛体最近傍→領域制限→BFSリム分割→アイランド多数決→関節ジュエル強制（浮遊解消も融合膜残存）／防水リメッシュ+Bone Heat（走り・射撃はクリーン化、脇膜がテント）／細ボクセル=開板の二重壁レース化・粗ボクセル=穴／ベイク4方式（Cycles画像・Cycles頂点・BVH最近傍・外側レイキャスト）は多重シェル内面の干渉で全滅、側面投影は遮蔽で破綻、ゾーン塗りで色は解決するが土台の斑が幾何起因と判明／腕分離リメッシュ（3分割）はスカルプトAPI版で密度不安定。**唯一の良好組合せ=全体一括モディファイアリメッシュ+Cyclesベイク**だが融合膜アーティファクトが残る。
  - **再挑戦の正道（次回タスク）**: 上位ティアRodin/Tripo等の**マニフォールド一枚メッシュ**で再生成すれば、構築済みパイプライン（`gen_v1/build_clean_rig.py`: ランドマークフィット・axesロール継承・二段バインド・`export_game_sprites_gen.py`）がそのまま機能する見込み。素材・スクリプト・実測ノウハウは `blender/hero/gen_v1/` に全保全。
- 復帰: `git checkout 8f975f7 -- index.html assets/player3d_*.png` でシェル版ヒーロー＋全モーション改修（ワイドストライド走行・戦闘構え・照準系）に戻した。ASSET_VER=2026071804。43/48/30全通過・実機で idle/run/shoot クリーン確認。
- 注意: `assets/player3d_anchors.json`（未追跡）は生成キャラ版のまま＝インラインPLAYER3D_MUZZLEとは不整合（実行時は未参照で無害）。シェル版で再書き出しすれば同期する。
- 公開: 未デプロイ。**現ローカル=デプロイ推奨状態**（3コミット分のモーション改修込み）。

## 2026-07-20 — [claude] hero-gen: 承認済みキャラマスターから3D主人公を再生成（ローカル反映・デプロイ待ち）
- 経緯: ユーザー「作画からやり直し」→ 調査で判明: 承認済みレンダー（`docs/mock/character-master.png`＋三面図・7/15承認）は正しく、壊れていたのは絵→3D粘土の手作業翻訳工程。承認画像から直接3D生成する方式へ転換（ユーザー承認済・トライアル枯渇時停止の条件付き）。
- 生成: Hyper3D Rodin 無料トライアル（BlenderMCPアドオン・MAIN_SITE/Sketchティア・1ジョブのみ消費）。入力=三面図3方向＋マスター3/4（`blender/hero/gen_inputs/`）。出力25,781頂点・テクスチャ付き→ `blender/hero/gen_v1/cobalt_hero_gen_v1.blend`。
- リグ接続（`blender/hero/gen_v1/build_gen_rig.py`・再現可能）:
  - 旧アーマチュアCOBALT_HERO_RIGを流用し、メッシュ実測ランドマーク（股下/膝/足首/両腕クラスタ）へ骨をリフィット。**ボーンロールは旧リグから継承**（ポーズテーブルはローカル軸前提のため必須）。左腕は右腕と非対称（体幹密着）なので独自クラスタでフィット。
  - スキニング: Bone Heatは非多様体で失敗するため**接続成分（装甲パーツ）単位のバインド**を自作: シード付きBFSでリム分割→3,671アイランドを最近チェーン平均距離で投票→関節ジュエルは遠位骨へ強制。ブレンドは同一チェーン内のみ。
  - 背中フィンはマスター比で過大だったため付け根へ42%縮小（`FIN_SCALE`）。orientation/scale はメッシュデータ直接変換（背景モードの ops transform_apply は回転が乗らない罠あり・要注意）。
  - 書き出し: `export_game_sprites_gen.py`（カメラ契約・ポーズ・アンカー計算は既存と同一。パレット上書きなし＝生成テクスチャ使用・AgX/露出0.55）。
- 検証: validate_game_sprites 8/124 OK、player3dcheck 43/43、posecheck 48/48、harness 30/30。実機（`?v=2026072003`）で idle=戦闘構え／run=前傾ワイドストライド／shoot=バスター水平＋砲口一致／jump 全て新キャラで表示・console error 0。
- **既知の課題**: (a) 右肘の小ジュエル1アイランドが腕曲げで数px浮く（実寸ほぼ不可視・次回対応） (b) 肩・腰に微小な継ぎ目シュレッド (c) 旧シェル前提の audit_motion_cycles / validate_motion_readability はスキンメッシュに未対応＝今回未実行（レンダー検証＋既存node検査で代替。監査のメッシュ対応は別タスク）。
- 旧素材への復帰: `git checkout 8f975f7 -- assets index.html`（旧3D素材一式）。旧リグ資産は rig_v1/ に不変。
- 公開: **未デプロイ**（検収待ち）。次: ユーザー実操作評価 → OKなら全改修（走り+立ち+新キャラ）をまとめてデプロイ。
